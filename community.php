...
if (isset($_POST['submit_post'])) {
    $author    = isset($_SESSION['user_id']) ? $_SESSION['username'] : trim($_POST['author_name']);
    ...
    if (!empty($author) && !empty($content)) {
        ...
        $stmt = $pdo->prepare("INSERT INTO feed_posts (author_name, author_id, hotel_id, content, image_url) VALUES (?, ?, ?, ?, ?)");
        $stmt->execute([$author, $author_id, $hotel_id, $content, $image_url]);
    ...
    <div style="background: #fff; padding: 20px; border-radius: 8px; box-shadow: 0 1px 4px rgba(0,0,0,0.1); margin-bottom: 20px;">
        ...
        <form action="" method="POST" enctype="multipart/form-data">
            <?php if (isset($_SESSION['user_id'])): ?>
                <div style="padding:10px; background:#f4f5f7; border-radius:6px; margin-bottom:10px; font-size:14px;">
                    Đăng với tên: <b><?= htmlspecialchars($_SESSION['username']) ?></b>
                </div>
            <?php else: ?>
